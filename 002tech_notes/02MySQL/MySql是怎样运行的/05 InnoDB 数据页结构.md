...
## Page Directory
### 结构
Page Directory 是一个数组，数组中的每个元素称为一个`槽`(slot)。每个槽存储的是某一行记录的真实数据bu f相对于页的起始位置的偏移量，这个偏移量指向的是一个记录的真实数据部分。
将页中所有记录按主键顺序排列，可以将连续的几个记录组成一组。
每组最大的那条记录的真实数据部分相对于页的起始位置的偏移量
