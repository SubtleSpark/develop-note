...
A：
1. 原先的老系统无法接入 Nacos 等新的开源方案。它只提供了 HTTP、TCP 的接口，无法直接接入 Dubbo 等 RPC 框架。原来只有一个接口，通过请求的 functionId 来区分不同的业务。中台希望只用修改一个服务器 ip，就能兼容新旧系统切换。所以自研系统需要支持这种调用方式
2. 自研网关在业务上有更多的定制化需求。比如，数据迁移时判断一个用户是否已经迁移，不是自研的用户就返回错误码。
